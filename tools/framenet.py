"""framenet — thin CLI for FrameNet frame aggregation (FR-4.5).

Aggregates a word-list CSV (first column) to first FrameNet frames.
Needs the optional ``wordnet`` extra (NLTK) plus the FrameNet 1.7 corpus.
"""
...
from core.analysis.framenet import aggregate, category_counts
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="FrameNet frame aggregation (words CSV)")
    ...
    parser.add_argument("--pos", choices=["NOUN", "VERB"], default="NOUN")
...
def _read_words(path: Path) -> list[str]:
    ...
    try:
        ...
        exit_with_diagnostics([Diagnostic.error("FRAMENET_WORDS_UNREADABLE", f"cannot read words CSV {path}: {exc}")])
        raise SystemExit(1) from exc  # pragma: no cover — exit_with_diagnostics always raises
    if frame.shape[1] < 1:
        exit_with_diagnostics([Diagnostic.error("FRAMENET_WORDS_EMPTY", f"words CSV {path} has no columns")])
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = aggregate(_read_words(args.words), pos=args.pos)
    ...
    if counted.value is None:  # unreachable: frame just came from aggregate
        exit_with_diagnostics(counted.diagnostics, code=1)
        return 1  # pragma: no cover
    try:
        writer = OutputWriter(args.output, tool="framenet", params=vars(args), inputs=(args.words,), corpus=None)
    except (ValueError, FileExistsError) as exc:
        print(f"writer error: {exc}", file=sys.stderr)
        return 1
    for artifact, name, description in (
        (frame, "framenet.csv", "FrameNet frame aggregation"),
        (counted.unwrap(), "framenet_frequency.csv", "FrameNet frame frequencies"),
    ):
    ...
    if not env.ok:
        ...
        return 1
    print(f"Wrote {len(frame)} word(s) to {writer.run_dir / 'framenet.csv'}")
    return 0
