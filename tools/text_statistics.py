"""text_statistics — thin CLI for per-document text statistics."""
...
from core.analysis.text_statistics import run
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    return build_common_parser("Per-document text statistics (sentence/word/syllable)").parse_args(argv)
...
def main(argv: list[str] | None = None) -> int:
    ...
    writer = make_writer(args, corpus, tool="text_statistics", params=vars(args))
    written = writer.write_table(frame, "text_stats.csv", kind="table", description="per-document text statistics")
    if not written.ok:
    ...
    if not env.ok:
        for d in env.diagnostics:
            print(d, file=sys.stderr)
        return 1
    print(f"Wrote {len(frame)} document(s) to {writer.run_dir / 'text_stats.csv'}")
