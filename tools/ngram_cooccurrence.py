"""ngram_cooccurrence — thin CLI for window co-occurrences."""
...
from core.analysis.ngram_cooccurrence import run
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = build_common_parser("N-gram co-occurrences within window")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run(table, window=args.window, field=field, min_count=args.min_count)
    if not result.ok:
        for d in result.diagnostics:
            print(d, file=sys.stderr)
        return 1
    frame = result.unwrap()
    writer = make_writer(args, corpus, tool="ngram_cooccurrence", params=vars(args))
    written = writer.write_table(frame, "cooccurrence.csv", kind="table", description="window co-occurrences")
    if not written.ok:
    ...
    if not env.ok:
        for d in env.diagnostics:
            print(d, file=sys.stderr)
        return 1
    print(f"Wrote {len(frame)} co-occurrence(s) to {writer.run_dir / 'cooccurrence.csv'}")
