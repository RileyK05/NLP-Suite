"""semantic — thin CLI for WordNet/VerbNet/FrameNet stub."""
...
from core.analysis.semantic import aggregate
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = build_common_parser("Semantic aggregation (WordNet/VerbNet/FrameNet stub)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = aggregate(table, field=field)
    if not result.ok:
        for d in result.diagnostics:
            print(d, file=sys.stderr)
        return 1
    frame = result.unwrap()
    writer = make_writer(args, corpus, tool="semantic", params=vars(args))
    written = writer.write_table(frame, "semantic.csv", kind="table", description="semantic tags")
    if not written.ok:
    ...
    if not env.ok:
        for d in env.diagnostics:
            print(d, file=sys.stderr)
        return 1
    print(f"Wrote {len(frame)} lemma(s) to {writer.run_dir / 'semantic.csv'}")
