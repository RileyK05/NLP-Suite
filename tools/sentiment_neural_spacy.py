"""sentiment_neural_spacy — thin CLI for neural sentiment via spaCy textcat."""
...
from core.analysis.sentiment_neural import spacy_sentences, summarize_neural
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = build_common_parser("Neural sentiment (spaCy textcat head)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = spacy_sentences(table, model=args.model)
    ...
    writer = make_writer(args, corpus, tool="sentiment_neural_spacy", params=vars(args))
    ...
    writer.add_diagnostics(
        ...
        *summary.diagnostics,
        Diagnostic.info(
            "SENTIMENT_NN_BACKEND", f"backend spacy, model {args.model}", backend="spacy", model=args.model
        ),
    )
    env = writer.finalize()
    if not env.ok:
