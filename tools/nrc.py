"""nrc — thin CLI for NRC emotion proportions (FR-4.3)."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = build_common_parser("NRC emotion proportions (full lexicon)")
    p.add_argument("--field", choices=["form", "lemma"], default="lemma")
    p.add_argument("--lexicon", type=Path, default=None, help="NRC JSON (defaults to asset, then nrclex bundled)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = emotions(table, field=field, lexicon=args.lexicon)
    if not result.ok:
        for d in result.diagnostics:
            print(d, file=sys.stderr)
        return 1
    frame = result.unwrap()
    writer = make_writer(args, corpus, tool="nrc", params=vars(args))
    written = writer.write_table(frame, "nrc.csv", kind="table", description="NRC emotion proportions")
    if not written.ok:
    ...
    if not env.ok:
        for d in env.diagnostics:
            print(d, file=sys.stderr)
        return 1
    print(f"Wrote {len(frame)} document(s) to {writer.run_dir / 'nrc.csv'}")
