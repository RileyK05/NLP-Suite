"""sentiment_neural_stanza — thin CLI for neural sentiment via Stanza's sentiment processor."""
...
from core.analysis.sentiment_neural import stanza_sentences, summarize_neural
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = build_common_parser("Neural sentiment (Stanza sentiment processor)")
    # --language comes from the shared parser arguments: the Stanza language
    # package and the corpus language are the same choice.
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = stanza_sentences(table, language=args.language)
    ...
    writer = make_writer(args, corpus, tool="sentiment_neural_stanza", params=vars(args))
    ...
    writer.add_diagnostics(
        ...
        *result.diagnostics,
        *summary.diagnostics,
        Diagnostic.info("SENTIMENT_NN_BACKEND", f"backend stanza, language {args.language}", backend="stanza"),
    )
    env = writer.finalize()
    if not env.ok:
